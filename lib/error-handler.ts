...
export function createAPIError(
  ...
  code: string,
  message: string,
  options: Partial<Omit<APIError, "type" | "code" | "message" | "timestamp">> = {},
): APIError {
  // 根据错误类型推断错误来源和严重程度
...
// 从各种错误类型创建API错误
export function handleError(error: unknown, defaultType = ErrorType.UNKNOWN_ERROR): APIError {
  // 如果已经是APIError，直接返回
  if (typeof error === "object" && error !== null && "type" in error && "code" in error && "message" in error) {
  ...
  if (error instanceof Error) {
    ...
    if (
      ...
      return createAPIError(ErrorType.NETWORK_ERROR, "NETWORK_FAILURE", "网络连接错误，请检查您的网络连接", {
        detail: errorMessage,
      })
    }

    // 检查是否是超时错误
    if (errorMessage.includes("timeout") || errorMessage.includes("超时") || error.name === "TimeoutError") {
      return createAPIError(ErrorType.TIMEOUT_ERROR, "REQUEST_TIMEOUT", "请求超时，请稍后重试", {
        detail: errorMessage,
        retryable: true,
      })
    }
    ...
    // 检查是否是外部API错误
    if (errorMessage.includes("API") || errorMessage.includes("api") || error.name === "ApiError") {
      return createAPIError(ErrorType.EXTERNAL_API_ERROR, "EXTERNAL_API_FAILURE", "外部服务暂时不可用，请稍后重试", {
        detail: errorMessage,
        retryable: true,
      })
    }
    ...
    // 默认作为服务器错误处理
    return createAPIError(defaultType, "INTERNAL_ERROR", "服务器内部错误，请稍后重试", { detail: errorMessage })
  }
  ...
  // 处理未知错误类型
  return createAPIError(ErrorType.UNKNOWN_ERROR, "UNKNOWN_ERROR", "发生未知错误，请稍后重试", {
