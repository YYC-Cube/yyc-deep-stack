...
import { handleApiError, generateErrorResponse, type ErrorType } from "./enhanced-error-handler"
  ...
  return async (request: NextRequest) => {
    ...
    try {
      ...
      if (!healthCheck.isHealthy && healthCheck.message) {
        ...
        return generateErrorResponse(healthCheck.message.message, healthCheck.message.httpStatus)
      }
    } catch (error: any) {
      // 5. 统一错误处理
      return handleApiError(error, service, requestUrl, requestMethod, startTime)
    }
  }
}
