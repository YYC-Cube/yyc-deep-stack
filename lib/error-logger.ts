...
class ErrorLogger {
  ...
  generateErrorReport(): ErrorReport {
    ...
    // 按严重程度统计
    const criticalErrors = recentLogs.filter((log) => log.severity === ErrorSeverity.CRITICAL).length
    const highErrors = recentLogs.filter((log) => log.severity === ErrorSeverity.HIGH).length
    const mediumErrors = recentLogs.filter((log) => log.severity === ErrorSeverity.MEDIUM).length
    const lowErrors = recentLogs.filter((log) => log.severity === ErrorSeverity.LOW).length
    ...
    if (increasingTrends.length > 0) {
      ...
      recommendations.push(`${topIncreasing.errorType}类型的错误呈上升趋势，建议优先调查。`)
    }
    ...
    topErrors.forEach((error) => {
      switch (error.errorType) {
        case ErrorType.EXTERNAL_API_ERROR:
          recommendations.push(`外部API错误频繁发生，建议检查API服务状态或实现更强的降级策略。`)
          break
        case ErrorType.DATABASE_ERROR:
          recommendations.push(`数据库错误频繁发生，建议检查数据库连接和查询性能。`)
          break
        case ErrorType.VALIDATION_ERROR:
          recommendations.push(`输入验证错误频繁发生，建议改进客户端验证或提供更清晰的输入指南。`)
          break
        case ErrorType.TIMEOUT_ERROR:
          recommendations.push(`请求超时错误频繁发生，建议优化性能或增加超时阈值。`)
          break
      }
    })
    ...
    if (recommendations.length === 0 && recentLogs.length > 0) {
      recommendations.push(`过去24小时内有${recentLogs.length}个错误，建议定期检查错误日志。`)
    }
