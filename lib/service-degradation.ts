...
class ServiceDegradation {
  ...
  private checkForDegradation(serviceName: string): void {
    ...
    // 如果错误数超过阈值，降级服务
    if (status.errorCount >= status.errorThreshold && status.health === ServiceHealth.HEALTHY) {
  ...
  private degradeService(serviceName: string): void {
    ...
    // 更新服务状态
    status.health = config.fallbackAvailable ? ServiceHealth.DEGRADED : ServiceHealth.UNAVAILABLE
  ...
  attemptRecovery(serviceName: string): boolean {
    ...
    if (status.recoveryAttempts > this.MAX_RECOVERY_ATTEMPTS) {
      const backoffMultiplier = Math.min(status.recoveryAttempts - this.MAX_RECOVERY_ATTEMPTS + 1, 10)
      status.nextRecoveryTime = Date.now() + this.RECOVERY_INTERVAL * backoffMultiplier
      return false
    }
  ...
  private isCriticalError(errorType: ErrorType): boolean {
    return [ErrorType.DATABASE_ERROR, ErrorType.RESOURCE_ERROR, ErrorType.SERVER_ERROR].includes(errorType)
  }
  ...
  cleanup(): void {
    ...
    for (const status of this.status.values()) {
      ...
      if (status.lastErrorTime && now - status.lastErrorTime > this.ERROR_WINDOW) {
        status.errorCount = 0
      }

      // 如果服务处于降级状态，尝试恢复
      if (status.health !== ServiceHealth.HEALTHY && status.nextRecoveryTime && now >= status.nextRecoveryTime) {
