...
class AIRecommendationEngine {
  ...
  async getRecommendations(
    ...
    try {
      // 输入验证
      if (!userId) {
        throw createAPIError(ErrorType.VALIDATION_ERROR, "MISSING_USER_ID", "请提供用户ID")
      }
      ...
      // 缓存检查
      const cacheKey = { userId, type, algorithm, limit, excludeViewed, diversityWeight, noveltyWeight }
      const cached = await getFromCache<RecommendationResult>("ai_recommendations", cacheKey)
      if (cached) {
        return cached
      }
      ...
      // 执行推荐算法
      const result = await this.performRecommendation(userId, userProfile, type, algorithm, {
        limit,
        excludeViewed,
        diversityWeight,
        noveltyWeight,
      ...
      // 记录API调用
      enhancedRecordAPICall("ai_recommendations", "/api/ai/recommendations", "POST", 200, Date.now() - startTime, {
        cacheHit: false,
        userId,
      ...
      enhancedRecordAPICall("ai_recommendations", "/api/ai/recommendations", "POST", 500, Date.now() - startTime, {
  ...
  async recordUserAction(action: UserAction): Promise<void> {
    ...
    try {
      ...
      if (!action.userId || !action.itemId || !action.actionType) {
        throw createAPIError(ErrorType.VALIDATION_ERROR, "INVALID_ACTION", "用户行为数据不完整")
      }
      ...
      // 记录API调用
      enhancedRecordAPICall("user_action_recording", "/api/ai/user-action", "POST", 200, Date.now() - startTime, {
        userId: action.userId,
        actionType: action.actionType,
      })
    } catch (error) {
      enhancedRecordAPICall("user_action_recording", "/api/ai/user-action", "POST", 500, Date.now() - startTime, {
        error: error instanceof Error ? error.message : String(error),
        userId: action.userId,
      })
      throw error
  ...
  private async performRecommendation(
    ...
    switch (algorithm) {
      case RecommendationAlgorithm.COLLABORATIVE_FILTERING:
        items = await this.collaborativeFiltering(userId, userProfile, type, options)
        break

      case RecommendationAlgorithm.CONTENT_BASED:
        items = await this.contentBasedFiltering(userId, userProfile, type, options)
        break

      case RecommendationAlgorithm.HYBRID:
        items = await this.hybridRecommendation(userId, userProfile, type, options)
        break

      case RecommendationAlgorithm.DEEP_LEARNING:
        items = await this.deepLearningRecommendation(userId, userProfile, type, options)
        break

      case RecommendationAlgorithm.MATRIX_FACTORIZATION:
        items = await this.matrixFactorization(userId, userProfile, type, options)
        break

      default:
        throw createAPIError(ErrorType.VALIDATION_ERROR, "INVALID_ALGORITHM", "不支持的推荐算法")
    }

    // 应用多样性和新颖性调整
    items = this.applyDiversityAndNovelty(items, options.diversityWeight, options.noveltyWeight)
  ...
  private async contentBasedFiltering(
    ...
    for (let i = 0; i < options.limit * 2; i++) {
      ...
      items.push({
        ...
        category: Object.keys(userProfile.preferences.categories)[i % 4],
        tags: [interest, ...Object.keys(userProfile.preferences.tags).slice(0, 2)],
        score,
  ...
  private async hybridRecommendation(
    ...
    // 获取不同算法的推荐结果
    const cfItems = await this.collaborativeFiltering(userId, userProfile, type, { limit: options.limit })
    const cbItems = await this.contentBasedFiltering(userId, userProfile, type, { limit: options.limit })
   ...
   * 计算新颖性分数
   */
  private calculateNoveltyScore(items: RecommendationItem[], userProfile: UserProfile): number {
  ...
  private generateExplanations(
    ...
    // 基于推荐结果生成解释
    const topCategories = [...new Set(items.slice(0, 5).map((item) => item.category))]
   ...
   * 更新用户画像
   */
  private async updateUserProfile(userId: string, action: UserAction): Promise<void> {
    // 获取当前用户画像
