...
class APIMonitor {
  ...
  private checkAlerts(metric: APIMetrics) {
    ...
    const recentMetrics = this.metrics.filter(
      (m) => m.serviceName === serviceName && m.timestamp > Date.now() - 5 * 60 * 1000,
    )
    ...
    if (errorRate > 0.1 && recentMetrics.length > 10) {
      this.addAlert("error", serviceName, `${serviceName}服务错误率过高: ${(errorRate * 100).toFixed(1)}%`)
    }

    // 响应时间警报
    const avgResponseTime = recentMetrics.reduce((sum, m) => sum + m.responseTime, 0) / recentMetrics.length
    if (avgResponseTime > 5000) {
      this.addAlert("warning", serviceName, `${serviceName}服务响应时间过长: ${avgResponseTime.toFixed(0)}ms`)
    }
  }

  // 添加警报
  private addAlert(type: "error" | "warning" | "info", service: string, message: string) {
    const alertId = `${service}-${type}-${Date.now()}`

    // 检查是否已存在相似警报
    const existingAlert = this.alerts.find(
      (a) => a.service === service && a.type === type && !a.resolved && Date.now() - a.timestamp < 10 * 60 * 1000, // 10分钟内
    )
  ...
  getServiceStats(serviceName: string): ServiceStats {
    ...
    const successRequests = serviceMetrics.filter((m) => m.statusCode < 400).length
    const errorRequests = serviceMetrics.filter((m) => m.statusCode >= 400).length
    ...
    return {
      ...
      errorRate: serviceMetrics.length > 0 ? errorRequests / serviceMetrics.length : 0,
      cacheHitRate: serviceMetrics.length > 0 ? cacheHits / serviceMetrics.length : 0,
      lastHour,
      lastDay,
    }
  }
  ...
  getMonitoringData(): MonitoringData {
    const services = ["weather", "news", "ipinfo", "currency", "stock", "geocode", "translate", "qrcode"]
