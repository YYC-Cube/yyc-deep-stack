...
export function enhancedRecordAPICall(
  ...
  // 增强监控：性能阈值检查
  checkPerformanceThresholds(serviceName, statusCode, responseTime, options.cacheHit)
}
...
export function getRecentAlerts(
  ...
  if (options.service) {
    filteredAlerts = filteredAlerts.filter((a) => a.service === options.service)
  }
...
export function getServiceHealthScore(service: string): number {
  ...
  if (serviceStats.averageResponseTime > thresholds.responseTime.critical) {
    score -= 25
  } else if (serviceStats.averageResponseTime > thresholds.responseTime.warning) {
    score -= 10
  }
...
export function generateHealthReport(): any {
  ...
  return {
    ...
    overallHealth: {
      ...
      status: healthScore.score > 80 ? "健康" : healthScore.score > 60 ? "需注意" : "不健康",
    },
    serviceHealth: healthScore.services,
    recentAlerts: getRecentAlerts({ limit: 5 }),
    performance: {
      averageResponseTime:
        Object.values(monitoringData.services).reduce((sum, s: any) => sum + s.averageResponseTime, 0) /
        Object.keys(monitoringData.services).length,
      totalRequests: Object.values(monitoringData.services).reduce((sum, s: any) => sum + s.totalRequests, 0),
      errorRate:
        Object.values(monitoringData.services).reduce((sum, s: any) => sum + s.errorRequests, 0) /
        Object.values(monitoringData.services).reduce((sum, s: any) => sum + s.totalRequests, 0),
    },
