...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      enabledServices: Object.values(apiStatus).filter((service) => service.enabled).length,
      demoServices: Object.values(apiStatus).filter((service) => service.demo).length,
    })
  } catch (error) {
    console.error("API状态检查错误:", error)
    return NextResponse.json({ error: "获取API状态失败", code: "SERVER_ERROR" }, { status: 500 })
  }
}
