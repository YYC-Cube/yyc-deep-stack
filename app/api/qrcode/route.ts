...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!content) {
      return NextResponse.json({ error: "请提供二维码内容", code: "MISSING_CONTENT" }, { status: 400 })
    }

    // 验证格式
    if (!["svg", "png", "data-url"].includes(format)) {
      return NextResponse.json(
        { error: "不支持的格式，请使用 svg, png 或 data-url", code: "INVALID_FORMAT" },
        { status: 400 },
      )
    }
    ...
    if (!["L", "M", "Q", "H"].includes(errorCorrectionLevel)) {
      return NextResponse.json(
        { error: "不支持的纠错级别，请使用 L, M, Q 或 H", code: "INVALID_ERROR_CORRECTION" },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json({ error: "生成二维码时发生错误", code: "GENERATION_ERROR" }, { status: 500 })
  }
}
