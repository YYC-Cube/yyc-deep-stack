...
export async function POST(request: NextRequest) {
  return withErrorHandling(
    ...
    async () => {
      ...
      // 验证必需参数
      if (!prompt) {
        throw createAPIError(ErrorType.VALIDATION_ERROR, "MISSING_PROMPT", "请提供生成提示")
      }

      if (!generationType) {
        throw createAPIError(ErrorType.VALIDATION_ERROR, "MISSING_GENERATION_TYPE", "请指定生成类型")
      }
      ...
      // 执行文本生成
      const result = await aiTextEngine.generateText(prompt, generationType, options)
