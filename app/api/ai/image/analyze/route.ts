...
export async function POST(request: NextRequest) {
  return withErrorHandling(
    ...
    async () => {
      ...
      if (!analysisType) {
        throw createAPIError(ErrorType.VALIDATION_ERROR, "MISSING_ANALYSIS_TYPE", "请指定分析类型")
      }
      ...
      // 执行图像分析
      const result = await aiImageEngine.analyzeImage(imageFile, analysisType, options)
