...
import { safeRemoveNode, safeAppendChild, safeDownloadFile } from "@/lib/dom-utils"
...
export function useSafeDOM() {
  ...
  // 安全添加元素
  const safeAppend = useCallback((parent: Node | null, child: Node | null): boolean => {
    if (!mountedRef.current) {
  ...
  // 安全下载文件
  const safeDownload = useCallback((data: string, filename: string, mimeType?: string): void => {
    if (!mountedRef.current) {
