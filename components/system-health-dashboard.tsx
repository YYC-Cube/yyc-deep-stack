...
import { CheckCircle, AlertCircle, AlertTriangle, Clock, RefreshCw } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>系统运行时间: {formatDuration(uptime * 1000)}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{health.overallHealth.score}/100</div>
                <Progress
                ...
                />
                <div className="mt-2 text-sm text-gray-500">状态: {health.overallHealth.status}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{health.performance.totalRequests}</div>
                <div className="mt-2 text-sm text-gray-500">
                  错误率: {(health.performance.errorRate * 100).toFixed(2)}%
                </div>
                <div className="mt-1 text-sm text-gray-500">
                  平均响应时间: {health.performance.averageResponseTime.toFixed(0)}ms
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-medium">{health.systemResources.memoryUsage.toFixed(1)} MB</span>
                </div>
                <Progress value={Math.min(health.systemResources.memoryUsage / 5, 100)} className="h-1 mb-3" />

                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm">CPU使用</span>
                  <span className="text-sm font-medium">{health.systemResources.cpuUsage.toFixed(1)}%</span>
                </div>
                <Progress value={health.systemResources.cpuUsage} className="h-1" />
              </CardContent>
            </Card>
          </div>
        </CardContent>
                  ...
                  <Card key={name}>
                    <CardHeader className="pb-2 flex flex-row items-center justify-between">
                      <div>
                        ...
                        <CardDescription>健康评分: {health.serviceHealth[name] || 0}/100</CardDescription>
                      </div>
                      <Badge className={healthColorMap[service.health] || "bg-gray-500"}>{service.health}</Badge>
                    </CardHeader>
                    <CardContent>
                      <div className="text-sm space-y-2">
                        ...
                        {service.degradedSince && (
                          <div className="flex justify-between">
                            <span>降级时长:</span>
                            <span>{formatDuration(Date.now() - service.degradedSince)}</span>
                          </div>
                        )}
                      </div>
                    </CardContent>
        ...
        <TabsContent value="cache">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                内存缓存大小: {(cache.stats.memoryCache.size / 1024).toFixed(2)} KB, 键数量:{" "}
                {cache.stats.memoryCache.keys}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {Object.entries(cache.serviceMetrics).map(([service, metrics]: [string, any]) => (
                  <Card key={service}>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-base">{service} 缓存</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="text-sm font-medium mb-1">命中率</div>
                          <div className="flex items-center">
                            <div className="flex-1 mr-2">
                              <Progress
                                value={(metrics.hits / (metrics.hits + metrics.misses)) * 100 || 0}
                                className="h-2"
                              />
                            </div>
                            <div className="text-sm">
                              {((metrics.hits / (metrics.hits + metrics.misses)) * 100 || 0).toFixed(1)}%
                            </div>
                          </div>
                        </div>
                        <div className="text-sm space-y-1">
                          ...
                          <div className="flex justify-between">
                            <span>总大小:</span>
                            <span>{(metrics.totalSize / 1024).toFixed(2)} KB</span>
                          </div>
                        </div>
                      </div>
                    </CardContent>
