        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              {lastUpdate && (
                <div className="text-white/60 text-sm">最后更新: {lastUpdate.toLocaleTimeString("zh-CN")}</div>
              )}
            </div>
            <div className="flex items-center space-x-2">
              ...
              <Button onClick={fetchMonitoringData} size="sm" className="text-white">
                <RefreshCw className="w-4 h-4 mr-2" />
                手动刷新
              </Button>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        <Card className="bg-gradient-to-br from-blue-400/20 to-blue-600/20 border-blue-300/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-white mb-1">{monitoringData.summary.totalServices}</div>
            <div className="text-blue-100 text-sm">总服务数</div>
            <div className="text-xs text-blue-200 mt-1">{monitoringData.summary.healthyServices} 健康</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-orange-400/20 to-orange-600/20 border-orange-300/30">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-orange-200 mt-1">
              {monitoringData.summary.overallErrorRate < 0.01 ? "优秀" : "需关注"}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-red-400/20 to-red-600/20 border-red-300/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-white mb-1">{monitoringData.summary.activeAlerts}</div>
      ...
      <Tabs defaultValue="services" className="w-full">
        ...
        <TabsContent value="services" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(monitoringData.services).map(([serviceName, stats]) => (
              <Card key={serviceName} className="bg-white/5 border-white/10">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-center">
                    <CardTitle className="text-white text-lg capitalize">{serviceName}</CardTitle>
                    <Badge className={`bg-gradient-to-r ${getServiceStatusColor(stats)} text-white border-0`}>
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-white font-semibold">{stats.totalRequests.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-white/60">成功率</div>
                      <div className="text-green-400 font-semibold">
                        {((stats.successRequests / stats.totalRequests) * 100 || 0).toFixed(1)}%
                      </div>
                    </div>
                    <div>
                      <div className="text-white/60">平均响应</div>
                      <div className="text-blue-400 font-semibold">{stats.averageResponseTime.toFixed(0)}ms</div>
                    </div>
                    <div>
                      <div className="text-white/60">缓存命中</div>
                      <div className="text-purple-400 font-semibold">{(stats.cacheHitRate * 100 || 0).toFixed(1)}%</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={stats.errorRate > 0.05 ? "text-red-400" : "text-green-400"}>
                        {(stats.errorRate * 100 || 0).toFixed(2)}%
                      </span>
                    </div>
                    <Progress value={stats.errorRate * 100} className="h-2 bg-white/20" />
                  </div>

                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-white/60">P95响应时间</span>
                      <span className="text-orange-400">{stats.p95ResponseTime.toFixed(0)}ms</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-white/60">P99响应时间</span>
                      <span className="text-red-400">{stats.p99ResponseTime.toFixed(0)}ms</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <div className="flex justify-between text-white/90 mb-2">
                    ...
                    <span className="font-bold">{monitoringData.systemHealth.cpuUsage.toFixed(1)}%</span>
                  </div>
                  <Progress value={monitoringData.systemHealth.cpuUsage} className="h-3 bg-white/20" />
                </div>
                ...
                <div>
                  <div className="flex justify-between text-white/90 mb-2">
                    ...
                    <span className="font-bold">{monitoringData.systemHealth.memoryUsage.toFixed(1)}MB</span>
                  </div>
                  <Progress
                    value={(monitoringData.systemHealth.memoryUsage / 1024) * 100}
                ...
                <div>
                  <div className="flex justify-between text-white/90 mb-2">
                    ...
                    <span className="font-bold">{(monitoringData.systemHealth.errorRate * 100).toFixed(2)}%</span>
                  </div>
                  <Progress value={monitoringData.systemHealth.errorRate * 100} className="h-3 bg-white/20" />
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="alerts" className="space-y-4">
          <Card className="bg-white/5 border-white/10">
            <CardHeader>
              ...
              <CardDescription className="text-white/80">系统自动检测到的异常情况和需要关注的问题</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-white/60 text-sm mt-1">
                              服务: {alert.service} • 时间: {new Date(alert.timestamp).toLocaleString("zh-CN")}
                            </div>
                          </div>
                        </div>
                        <Button
                  ...
                  {Object.entries(monitoringData.services)
                    ...
                    .map(([serviceName, stats]) => (
                      <div key={serviceName} className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-white/80 capitalize">{serviceName}</span>
                          <span className="text-white/60">{stats.totalRequests} 请求</span>
                        </div>
                        <div className="flex space-x-2">
                          <div className="flex-1">
                            <div className="text-xs text-white/60 mb-1">成功率</div>
                            <Progress
                              value={(stats.successRequests / stats.totalRequests) * 100 || 0}
                              className="h-2 bg-white/20"
                            />
                          </div>
                          <div className="flex-1">
                            <div className="text-xs text-white/60 mb-1">缓存命中</div>
                            <Progress value={stats.cacheHitRate * 100 || 0} className="h-2 bg-white/20" />
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="text-center p-3 bg-white/5 rounded-lg">
                      <div className="text-2xl font-bold text-blue-400 mb-1">
                        {Object.values(monitoringData.services)
                          .reduce((sum, service) => sum + service.totalRequests, 0)
                          .toLocaleString()}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-purple-400 font-semibold">
                        {Object.entries(monitoringData.services).reduce(
                          (highest, [name, stats]) =>
                            stats.cacheHitRate > highest.rate ? { name, rate: stats.cacheHitRate } : highest,
                          { name: "", rate: 0 },
                        ).name || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="text-cyan-400 font-semibold">
                        {Object.entries(monitoringData.services).reduce(
                          (busiest, [name, stats]) =>
                            stats.totalRequests > busiest.requests ? { name, requests: stats.totalRequests } : busiest,
                          { name: "", requests: 0 },
                        ).name || "N/A"}
                      </span>
                    </div>
